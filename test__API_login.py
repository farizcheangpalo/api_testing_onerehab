...
def test_post_organizations():
    ...
    assert response.status_code == 200
    assert response.json()["subOrgResponse"][0]["name"] == "OneRehab CSP"
    assert response.json()["subOrgResponse"][0]["code"] == "OneRehab_CSP"
    assert response.json()["subOrgResponse"][0]["orgCenterResponse"][0]["name"] == "OneRehab-CSP-Center"
    assert response.json()["subOrgResponse"][0]["orgCenterResponse"][0]["name"] == "OneRehab-CSP-Center"
...
# verify that active
def test_post_discharge_list():
    API_URL = "{}/{}".format(login.API_URL,url_ext.LIST)
    client = ServerApi(api_url=API_URL)
    request_json = util.read_json_file("data/json/request/discharge_list.json")
    response = client.post_with_bearer_token(request_json)
